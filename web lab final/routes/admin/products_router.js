...
// Add these routes for user dashboard
router.get('/user/dashboard', async (req, res) => {
    ...
    try {
        const user = await User.findById(req.session.userId);
        const wishlistItems = await User.findById(req.session.userId)
            .populate('wishlist');
...
// Add this route for men's page
router.get("/men", async (req, res) => {
    try {
        const products = await Product.find().populate('category');
        res.render("pages/main/men", { 
            products,
            layout: 'layout.ejs',
        ...
        res.render("pages/main/men", { products: [], error: "Error loading products" });
    }
});

// Add women's page route
